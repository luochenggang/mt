...
int OnInit()
{
   ...
   Print(">>> NAS100 Trend+Recovery+Manual (v5.0) Loaded. Recovery Var Name: ", g_GV_Name);
...
void OnTick()
{
   ...
   if(now >= tradeEndTime) {
      if(g_IsSystemActive || PositionsTotal() > 0 || OrdersTotal() > 0) {
         ...
         Print(">>> Trading session ended. Positions closed.");
      }
      return;
   }
...
void UpdateInfoPanel(datetime now) {
    ...
    // Build String
    string text = "---------- NAS100 GRID SYSTEM v5.1 ----------\n";
